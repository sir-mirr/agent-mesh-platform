...
| GET    | `/api/v1/admin/reminders/overdue` | JWT\*  | `200`   | Every `once` reminder the scheduler is **holding** for an operator (§ 3.3), and every decision already made. A one-shot more than the scheduler's hold window past its slot is not fired: the moment it was for has passed, there is no later slot to move it to, and delivering it late may be worse than dropping it — so a person decides. **Held is read from the scheduler's own record, never recomputed.** The window is the daemon's configuration, in a different process; a route applying its own threshold gives two answers to *is this held* that drift the moment either side is reconfigured. Holds and decisions come back together because the screen's question is *what is waiting and what has been answered*; split across two calls, a reader renders a hold as unanswered while the other call is still in flight. Requires `reminder.read.held`. |
| POST   | `/api/v1/admin/reminders/overdue/{id}/decision` | JWT\*  | `200`   | Decide one held slot: `{ scheduled_at, decision: "replay"｜"skip", approval_ref }`. **`replay` sends the slot's original content, once, unchanged**; `skip` means it never arrives. Neither rewrites the body — the operator is answering *this was supposed to go out and did not, should it still*, and a `replay` that altered the content would leave them unable to say what the recipient receives. **`scheduled_at` is required, is part of the key, and is the value the list published.** The route accepts the ISO-8601 instant `GET /api/v1/admin/reminders/overdue` gives for that hold and matches it against the scheduler's own slot, which is stored in SQLite's shape; a console has no second source for this field, and a route that only accepted the storage spelling answered `404` to every row it had just drawn. A decision belongs to one fire: between the screen drawing a hold and this request arriving the scheduler may have moved on, and keyed on the reminder alone the decision would land on a slot nobody looked at. A slot that is not held answers `404 NO_SUCH_HOLD` rather than recording a row the scheduler will never read. **`approval_ref` is required in substance and free in form.** `APPROVED:x` is valid — a product with no ticket system gains nothing from a format rule but the appearance of one, and the discipline comes from the string being shown beside the decision and its decider afterwards. A reference with nothing after the prefix is `400 EMPTY_APPROVAL_REF`: that is an absent answer wearing the shape of one. A body that is not JSON, names no `scheduled_at`, or carries a decision that is neither `replay` nor `skip` is `400 INVALID_DECISION` — one code for the shape of the request, because a caller fixes the request either way. The record carries **who decided**, so a decision can be asked about. Requires `reminder.decide`, which is separate from reading the list: seeing that a reminder is held is a different authorisation question from deciding what happens to it. |
...
| GET    | `/api/v1/admin/groups`            | JWT\*  | `200`   | Groups, their members and every egress rule (§ 12), for the caller's tenant — every tenant for the platform administrator, each row carrying `tenant`. |
| POST   | `/api/v1/admin/groups`            | JWT\*  | `201`\|`200` | Create one. It can send nowhere until a rule says so (§ 12). `200` when the group was already there: the route is idempotent, and nothing was created. Accepts `tenant`; it defaults to the caller's, and naming another is the platform administrator's (`403 TENANT_NOT_YOURS`, `400 NO_SUCH_TENANT`). |
| POST   | `/api/v1/admin/groups/{group_id}/members` | JWT\* | `200` | Move an identity in. Membership is singular (§ 12). Accepts `tenant`, resolved as above; `(tenant, group_id)` is the key, so a group of that name in another tenant is a different group and answers `404`. |
| POST   | `/api/v1/admin/groups/{group_id}/egress` | JWT\* | `201` | Allow `{group_id} -> to_group`. Directional (§ 12). Accepts `tenant`, resolved as above. |
| DELETE | `/api/v1/admin/groups/{group_id}/egress/{to_group}` | JWT\* | `200` | Withdraw that one direction (§ 12). The tenant comes from `?tenant=`, not a body: the target is in the path. |
...
| GET    | `/api/v1/admin/tenants/directory` | JWT    | `200`   | Which tenants there are — id, name, `deleted_at` — and which one this session is in. Any approved session; an ordinary one sees only its own tenant, the platform administrator sees all of them including deleted. Not gated on `tenant.read.stats`: that answers how much traffic a tenant received, this answers which tenants exist. |
| POST   | `/api/v1/admin/tenants`           | JWT\*  | `201`   | Create one, `{ id, name }`. Platform administrator only. `409 TENANT_EXISTS` for an id that is taken — including by a deleted tenant, whose id stays taken because other rows still name it. |
| PATCH  | `/api/v1/admin/tenants/{id}`      | JWT\*  | `200`   | Rename one, `{ name }`. Platform administrator only. The id is never rewritten: four tables hold it as a plain string. `404` for a tenant nobody created. |
| DELETE | `/api/v1/admin/tenants/{id}`      | JWT\*  | `200`   | Stop offering one. A soft delete — the row stays so the ids in `message_stats` and `local_users` still resolve to a name. Platform administrator only. Answers `action`: `deleted`, `already-deleted`, or `not-found`. `409 DEFAULT_TENANT` for `default`: every row whose tenant nobody stated is in it. |
...
| GET    | `/api/v1/admin/grants`            | JWT\*  | `200`   | Who holds which capability (§ 11). Gated on `role.grant`. Each row carries `revocable`, and `immutable_reason` (`last_grantor` or `protected_account`) when it is `false`. `immutable_subjects` lists the accounts whose every row is locked. |
| POST   | `/api/v1/admin/grants`            | JWT\*  | `201`   | Grant a capability to a subject (§ 11). |
| DELETE | `/api/v1/admin/grants`            | JWT\*  | `200`   | Revoke one. Absent is not an error (§ 11). `409 LAST_GRANTOR` if it would leave no tenant-wide `role.grant`; `409 PROTECTED_ACCOUNT` for a subject whose grants are re-seeded at startup. |
| GET    | `/api/v1/admin/keys/pending`      | JWT\*  | `200`   | Keys awaiting an approval decision (§ 10.2.1). |
| GET    | `/api/v1/admin/keys/stream`       | JWT\*  | `200`   | Key proposals as they arrive, SSE (§ 10.2.1). **A second source for the same fact as `/api/v1/admin/keys/pending`**, and a client is expected to hold both: the list on load, the stream after. A pending key therefore reaches the screen from whichever arrives first, so blocking one of them does not stop the count from filling and is not a measurement of the feature. `agent-mesh-local-pm` found this as a race — two runs of one tool against one commit disagreed on five pairs, all of them this route — and it is written here because the alternative is each reader rediscovering it from a flake. **Both sources send `keys`**: this stream opens with `snapshot` carrying `{ keys: [...] }`, the same field the list route answers with, and then sends `key-proposed` per arrival. Saying the two are one fact without saying what either sends is how they came to disagree — the list moved to `keys` and the stream stayed on `proposals`, so a client reading both got the same queue under two names. |
...
| `reminder.read.held` | which `once` reminders the scheduler is holding for a decision (§ 3.3) |
| `reminder.decide` | replay a held slot once, or skip it for good (§ 3.3) — **separate from reading the list**, because deciding acts on somebody else's mailbox |
