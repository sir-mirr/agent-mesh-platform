...
export const DICTIONARY: Record<Language, Record<string, string>> = {
  ko: {
    ...
    "dash.openHub": "허브 열기 →",
    "dash.egressLink": "🛡️ Egress ACL 설정",
    ...
    "pairing.modal.title": "🤖 AI 에이전트 등록 요청 & 페어링 인증",
    "play.dispatchTitle": "메시지 발송 (Outbox Dispatch)",
    ...
    "lease.what.title": "Lease (독점 임대) 상태란?",
    "lease.what.body": "비동기/서버리스 워커 에이전트가 메시지를 꺼내갈 때, 즉시 삭제하지 않고 다른 워커의 중복 처리를 방지하기 위해 일정 시간 동안 독점 잠금(Lock)하는 개념입니다.",
    "lease.ttl.title": "300초 TTL (At-Least-Once 안전장치)",
    "lease.ttl.body": "워커가 정상 완료 시 ACK 를 보내면 영구 해제되며, 만약 워커 프로세스가 비정상 종료되어 300초 동안 응답이 없으면 메시지가 자동 복구(Available)되어 유실 없이 재처리됩니다.",
    "lease.ack": "ACK 승인",
    "lease.nack": "NACK 반환",
    "lease.acquire": "Lease 획득",
    "lease.col.hub": "메시 허브",
    "lease.col.id": "메시지 ID",
    "lease.col.route": "경로 (발신 에이전트 → 수신 에이전트)",
    "lease.col.state": "리스 상태",
    "lease.col.count": "300s 리스 카운트다운",
    "lease.col.batch": "원자적 일괄 작업",
    ...
    "rbac.needs.grant": "role.grant 권한이 필요합니다",
    "rbac.subtitle": "SPEC § 11.3 / § 12: 계정별 Capability(권한) 세분화 부여 및 회수 (role.grant 인가 전용)",
    "rbac.matrix": "활성 조직원 및 Capability 권한 할당 매트릭스",
    "rbac.error": "RBAC 권한 데이터를 불러올 수 없습니다 (role.grant 권한 부족 또는 서버 오류).",
    ...
    "users.issued.once": "한 번만 보여준다. 이 화면을 떠나거나 새로고침하면 사라지고 서버는 다시 알려주지 않는다 — 다시 발급하려면 그 사람을 다시 들여야 한다.",
    "users.unreachable": "서버가 응답하지 않았다. 아무것도 만들어지지 않았다.",
    "users.error": "계정 목록을 읽지 못했다 (user.admit 권한이 없거나 서버가 거절했다).",
  ...
  en: {
    ...
    "pairing.modal.title": "🤖 Agent registration request and pairing",
    ...
    "lease.what.title": "What a lease is",
    ...
    "lease.ttl.title": "300s TTL (the at-least-once safeguard)",
    "lease.ttl.body": "An ACK from a worker that finished releases it for good. If the worker dies and nothing answers for 300 seconds the message returns to Available and is processed again rather than lost.",
    "lease.ack": "ACK",
    "lease.nack": "NACK",
    "lease.acquire": "Acquire lease",
    "lease.col.hub": "Mesh hub",
    "lease.col.id": "Message ID",
    "lease.col.route": "Route (from → to)",
    "lease.col.state": "Lease state",
    "lease.col.count": "300s countdown",
    "lease.col.batch": "Atomic batch action",
    ...
    "login.demo.fin": "Fin",
    "login.demo.pinja": "Pinja",
    "login.demo.areum": "Areum",
    "rbac.toast.revoked": "Revoked",
    "rbac.toast.granted": "Granted",
    "rbac.toast.failed": "Could not change the grant",
    "rbac.needs.grant": "role.grant is required",
    "rbac.subtitle": "SPEC § 11.3 / § 12: capabilities granted and revoked per account (role.grant only)",
    "rbac.matrix": "Members and their capabilities",
    "rbac.error": "Could not read the grants (role.grant required, or the server refused).",
    ...
    "users.issued.once": "Shown once. Leaving or reloading this page loses it, and the server will not repeat it — admit the person again to issue a new one.",
    "users.unreachable": "The server did not answer. Nothing was created.",
    "users.error": "Could not read the accounts list (user.admit required, or the server refused).",
