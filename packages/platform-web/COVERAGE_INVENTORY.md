...
| 계열 | 재는 것 | 등록 |
|---|---|---|
| `SC-DOWN-*` | 백엔드가 끊겼을 때 화면이 무엇을 말하는가 · **배포 위상에서는 세션 경로도 같이 끊긴다** · **한 라우트만 죽었을 때 못 물어본 것을 없다고 말하지 않는가** · **관리자 아닌 화면도 거절을 0 으로 그리지 않는가** · **그릴 수 없는 패널이 사라지지 않고 말하는가** | 15 |
| `SC-WRITE-*` | 실패한 쓰기를 성공으로 그리지 않는가 · **서버가 영수증을 안 주면 그렇게 말하는가** · **결정을 서버가 안 받았는데 결정됐다고 하지 않는가** · **되돌린 셀이 실제로 되돌아가는가** | 11 |
| `SC-ACT-*` | 액션이 서버 응답을 확인하는가 | 6 |
| `SC-AUTH-*` | 세션이 있는가, 만료되면 무엇을 말하는가 · **역할을 화면이 고르지 않는가** · **나가면 브라우저에 세션이 안 남는가** | 7 |
| `SC-LOAD-*` | 로딩·지연 상태 · **응답 전에 0 이라고 답하지 않는가** | 6 |
| `SC-NAV-*` | 메뉴가 보유 권한을 따르는가 | 4 |
| `SC-CAP-*` | 권한 없는 주체에게 무엇이 안 보이는가 · 안 보이는 이유를 말하는가 · **없는 제약을 주장하지 않는가** · **거르지 않은 목록을 그 사람의 것이라 부르지 않는가** · **거절과 못 닿음을 다른 문장으로 말하는가** · **되돌릴 수 없는 조작을 못 쓰는 세션에 내놓지 않는가** · **권한 하나만 든 세션이 가진 화면은 보고 못 가진 화면은 이유를 듣는가** · **거절을 통신 오류로 그리지 않는가** · **거절을 침묵이라 말하는 화면이 없는가**(전 화면) | 11 |
| `SC-API-AUTH-*` | 같은 질문을 API 층에서 | 3 |
| `SC-HARNESS-*` | 하네스가 잰다고 말한 것을 실제로 재는가 | 3 |
| `SC-ADDR-*` | 화면이 주소·지문을 못 아는데 아는 척하지 않는가 | 1 |
| `SC-VOCAB-*` | 어휘 | 1 |
| `SC-I18N-*` | 사전 대칭 · 랜딩이 읽히는가 · **화면이 부르는 키를 사전이 정의하는가** · **사전을 거치지 않은 한글이 소스에 있는가** · **영어로 연 화면이 제 문구를 한글로 그리는가**(런타임 · 출처로 데이터와 가름) | 5 |
| `SC-MODULE-*` | 번들 | 1 |
| `SC-PROV-*` | 출처 | 1 |
...
| `SC-INVENT-*` | **서버가 안 보낸 필드를 값으로 그리지 않는가** — 같은 필드를 보낸 판·뺀 판 둘로 잰다 · **한 라벨 아래 다른 표의 수를 답하지 않는가** · **요약 라우트의 행을 메시지로 지어내지 않는가** | 6 |
