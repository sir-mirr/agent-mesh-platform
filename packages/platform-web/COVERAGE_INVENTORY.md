...
| `SC-WRITE-*` | 실패한 쓰기를 성공으로 그리지 않는가 · **서버가 영수증을 안 주면 그렇게 말하는가** · **결정을 서버가 안 받았는데 결정됐다고 하지 않는가** · **되돌린 셀이 실제로 되돌아가는가** · **성공했다는 쓰기가 실제로 서버에 닿았는가** · **권한을 주는 쪽도 서버에 닿는가** · **키 승인이 서버에 닿는가** · **egress 허용이 실제로 써지는가** · **받아들여진 쓰기에 화면이 영수증을 그리는가** | 21 |
