...
| `SC-SIGNIN-*` | **배포가 실제로 완료할 수 있는 로그인만 제시하는가** — public health가 GitHub 비활성을 답하면 OAuth 버튼은 없고 로컬 로그인은 남는다 · 가입 요청 절은 요청 없음과 요청이 올 수 없음을 가른다 | 1 |
...
- **라우트**: `/login`
- **권한 요건**: Public
- **데이터 소스**: `POST /auth/login`, `GET /api/v1/health`
...
| GitHub 로그인 가시성 | 배포가 완료 가능한 OAuth 여부 | `/api/v1/health`의 `sign_in.github` | • `true`: GitHub 로그인 버튼과 로컬 계정 구분선 표시<br>• `false`·무응답·잘못된 몸통: 완료 가능하다고 추정하지 않고 버튼 미표시<br>• 어느 경우에도 로컬 로그인 폼 유지 | `SC-SIGNIN-01` |
