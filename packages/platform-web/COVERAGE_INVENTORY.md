...
| `SC-WRITE-*` | 실패한 쓰기를 성공으로 그리지 않는가 | 8 |
...
| `SC-HARNESS-*` | 하네스가 잰다고 말한 것을 실제로 재는가 | 2 |
| `SC-VOCAB-*` | 어휘 | 1 |
| `SC-MODULE-*` | 번들 | 1 |
| `SC-PROV-*` | 출처 | 1 |
...
있었으며, `SC-AUTH-*` 의 뒤쪽 둘과 `SC-HARNESS-*` 의 둘째는 행 자체가
없었습니다.

*(개별 ID 를 여기 적지 않는 것은 § 4 의 등록 수 검사가 **문서에 나타난 ID 를
등록으로 세기 때문**입니다. 설명문이 예시로 든 ID 도 등록으로 잡힙니다 —
`test/scenario-ids.test.ts` 의 해당 검사 주석에 그 한계를 적어 뒀습니다.)*
