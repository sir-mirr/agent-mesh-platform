...
| `SC-CAP-*` | 권한 없는 주체에게 무엇이 안 보이는가 · 안 보이는 이유를 말하는가 · **없는 제약을 주장하지 않는가** · **거르지 않은 목록을 그 사람의 것이라 부르지 않는가** · **거절과 못 닿음을 다른 문장으로 말하는가** · **되돌릴 수 없는 조작을 못 쓰는 세션에 내놓지 않는가** · **권한 하나만 든 세션이 가진 화면은 보고 못 가진 화면은 이유를 듣는가** | 9 |
