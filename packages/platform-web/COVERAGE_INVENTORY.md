...
| 계열 | 재는 것 | 등록 |
|---|---|---|
| `SC-DOWN-*` | 백엔드가 끊겼을 때 화면이 무엇을 말하는가 · **배포 위상에서는 세션 경로도 같이 끊긴다** | 12 |
| `SC-WRITE-*` | 실패한 쓰기를 성공으로 그리지 않는가 · **서버가 영수증을 안 주면 그렇게 말하는가** | 9 |
| `SC-ACT-*` | 액션이 서버 응답을 확인하는가 | 6 |
| `SC-AUTH-*` | 세션이 있는가, 만료되면 무엇을 말하는가 · **역할을 화면이 고르지 않는가** | 6 |
| `SC-LOAD-*` | 로딩·지연 상태 | 5 |
...
| `SC-VOCAB-*` | 어휘 | 1 |
| `SC-I18N-*` | 사전 대칭 · 랜딩이 읽히는가 · **화면이 부르는 키를 사전이 정의하는가** | 3 |
| `SC-MODULE-*` | 번들 | 1 |
| `SC-PROV-*` | 출처 | 1 |
| `SC-CONSIST-*` | **화면이 선언한 수가 화면이 그린 것의 개수인가** | 1 |
| `SC-USER-*` | **사람을 들이는 길** — 임시 비번을 한 번만 · 바꾸기 전엔 그것 말고 아무것도 · 권한 없이 시작 · 부여하면 열리고 회수하면 닫힌다 · 화면의 역할·권한 축은 서버가 말한 것 | 6 |
