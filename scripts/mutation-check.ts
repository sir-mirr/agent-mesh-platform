...
export const MUTATIONS: Mutation[] = [
  ...
  {
    id: "a-guarded-route-loses-its-capability",
    defect: "the audit route stops naming a capability, so it asks only for a session. The menu still hides the link from anybody who cannot use it, which is what makes this survivable to look at \u2014 the screen is one typed URL away from a reader the server never granted, and the route is the only thing that was ever going to refuse them.",
    file: "packages/platform-web/src/App.tsx",
    from: "                  <GuardedRoute requiredCapability=\"audit.read.metadata\">\n                    <AuditLogsPage />",
    to: "                  <GuardedRoute>\n                    <AuditLogsPage />",
    suite: "test/fe-render.test.ts",
    expect: ["SC-NAV-04"],
  },
